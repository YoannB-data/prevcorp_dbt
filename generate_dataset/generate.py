...
def gen_contracts(segments: list[dict], n: int = 500) -> list[dict]:
    ...
    for i in range(n):
        ...
        rows.append({
            ...
            "TYP_CONTRAT":    random.choices(TYP_CONTRAT_VALUES, TYP_CONTRAT_WEIGHTS)[0],
...
def gen_contract_companies(
    ...
    # Index contrats par type
    cadre_ctrs   = [c for c in contracts if c["TYP_CONTRAT"] == "CADRE"]
    nc_ctrs      = [c for c in contracts if c["TYP_CONTRAT"] == "NON CADRE"]
    edp_ctrs     = [c for c in contracts if c["TYP_CONTRAT"] == "ENSEMBLE DU PERSONNEL"]

    # Pour chaque contrat : 80% → 1 entreprise, 15% → 2, 5% → 3
    n_companies_weights = [0.80, 0.15, 0.05]
    company_ids = [c["COMPANY_ID"] for c in companies]

    # Suivi des contrats déjà attribués à chaque entreprise
    company_contracts: dict[str, list[str]] = {c["COMPANY_ID"]: [] for c in companies}
        ...
        for company_id in chosen:
            existing_types = [
                ...
                if (c["CONTRACT_ID"], company_id) in pairs
                   or any(r["CONTRACT_ID"] == c["CONTRACT_ID"] and r["COMPANY_ID"] == company_id for r in rows)
            ]
...
PAYS_RESIDENCE = (
    ["FR"] * 88
    + ["BE", "DE", "ES", "IT", "PT", "CH", "MA", "TN", "DZ", "SN", "CM"]
)
...
def gen_policyholders(n: int = 2000) -> list[dict]:
    ...
    for i in range(n):
        ...
        rows.append({
            ...
            "DT_NAISSANCE":    rand_date(date(1950, 1, 1), date(1995, 12, 31)).isoformat(),
...
COD_RISQUE_VALUES  = ["Incapacité", "Invalidité", "Décès", "Rente viagère", "Rente éducation"]
COD_RISQUE_WEIGHTS = [0.45,         0.25,         0.10,   0.10,            0.10]

# NAT_ARRET pertinent pour Incapacité et Invalidité uniquement
NAT_ARRET_VALUES  = ["Maladie", "Accident travail", "Accident de trajet", "Maladie Professionnelle", "Maternité"]
...
def gen_claims(
    ...
    for link in contract_companies:
        contract_to_companies.setdefault(link["CONTRACT_ID"], []).append(link["COMPANY_ID"])

    # ~95% des assurés ont au moins un dossier
    active_pols = random.sample(
        policyholders, k=int(len(policyholders) * 0.95)
    )
    ...
    for pol in active_pols:
        ...
        for _ in range(n_claims):
            ...
            if cod_risque == "Invalidité":
                dt_attest_na = rand_date(dt_creation, dt_etat_dos).isoformat() if random.random() < 0.70 else ""
                dt_justif_ss = rand_date(dt_creation, dt_etat_dos).isoformat() if random.random() < 0.65 else ""
            else:
                dt_attest_na = ""
                dt_justif_ss = ""
...
def _generate_payments_incapacite(claim: dict) -> list[dict]:
    ...
    dt_end   = END_DATE if not claim["MOTIF_ETAT_DOS"] else date.fromisoformat(claim["DT_ETAT_DOS"])
...
def gen_premiums(contracts: list[dict]) -> list[dict]:
...
def gen_beneficiaries(claims: list[dict]) -> list[dict]:
    ...
    rente_claims = [
        c for c in claims
        if c["COD_RISQUE"] in ("Rente viagère", "Rente éducation")
    ]
